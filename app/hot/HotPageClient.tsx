...
export function HotPageClient({ initialFilters, initialData }: HotPageClientProps) {
  ...
  const fetchHoles = async (offset = 0) => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        threshold: Math.max(filters.threshold, 0).toString(),
        offset: offset.toString(),
        limit: '20',
      });

      const response = await fetch(`/api/holes/hot?${params.toString()}`, {
        cache: 'no-store',
      });
      ...
      if (response.ok) {
        ...
        console.error('获取热点树洞失败:', await response.text());
      }
    } catch (error) {
  ...
  const handleThresholdChange = (value: string) => {
    if (value === 'custom') {
      ...
      setFilters((prev) => ({ ...prev, threshold: parseInt(value, 10) }));
      setCustomThreshold('');
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilters((prev) => ({ ...prev, timeRange: e.target.value as HotHoleFilters['timeRange'] }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilters((prev) => ({ ...prev, sortBy: e.target.value as HotHoleFilters['sortBy'] }))
                }
                className="w-full p-3 border-0 bg-[#f5f5f7] dark:bg-black rounded-xl text-black dark:text-white"
              >
